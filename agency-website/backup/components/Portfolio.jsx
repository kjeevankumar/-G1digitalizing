...
export default function Portfolio() {
  return (
    <motion.section 
      ...
      id="portfolio" 
      className="py-20 relative z-10 w-full overflow-hidden section-bg-photo"
    >
      <div className="container mx-auto px-12 w-full max-w-7xl">
        ...
        >
          ...
          <h2 className="text-md lg:text-6xl text-slate-900 font-bold tracking-tight leading-[1.05]">
        ...
        <FocusGridWrapper className="flex flex-col gap-24">
           {projects.map((proj, idx) => {
             ...
             return (
               <FocusCard id={idx} key={idx} className="w-full">
                 ...
                 viewport={{ once: true, margin: "-10%" }}
                 transition={{ duration: 0.6, ease: "easeOut" }}
                 className={`flex flex-row ${even ? 'md:flex-row' : 'md:flex-row-reverse'} items-center gap-10 md:gap-16 w-full`}
               >
                 ...
                 <div className="w-[45%] flex flex-col justify-center">
                    <div className="inline-block py-1 px-3 rounded-full bg-slate-100 text-slate-500 font-bold text-[6px] sm:text-[8px] md:text-[10px] uppercase tracking-widest mb-6 w-fit">
                      {proj.category}
                    </div>
                    <h3 className="text-base sm:text-md lg:text-5xl text-slate-900 font-bold tracking-tight mb-6 leading-tight">
                      {proj.title}
                    </h3>
                    <p className="text-[8px] sm:text-md lg:text-lg text-slate-500/90 font-medium leading-[1.4] md:leading-[1.6] max-w-lg mb-8">
                      {proj.description}
                    </p>
                    {proj.metrics && (
                      <div className="bg-blue-50/50 border border-blue-100/50 rounded-lg md:rounded-2xl p-6 mb-8 w-fit shadow-sm relative group/metric cursor-default">
                        <div className="flex items-center gap-4">
                           ...
                           <div className="text-slate-900 font-bold text-[8px] sm:text-[10px] md:text-lg pr-2 md:pr-4">
                             {proj.metrics}
                           </div>
                        </div>
                      </div>
                    ...
                    <DirectionalButton 
                      ...
                      className="inline-flex items-center justify-center bg-slate-900 text-white px-3 sm:px-8 py-4 rounded-md md:rounded-xl font-bold tracking-wide shadow-md hover:shadow-lg hover:shadow-blue-600/30 transition-shadow duration-[200ms] w-fit text-[8px] sm:text-[9px] md:text-[15px] cursor-pointer"
                    >
                      View Live Project
                    </DirectionalButton>
                 </div>
